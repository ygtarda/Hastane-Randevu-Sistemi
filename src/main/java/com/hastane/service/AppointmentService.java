...
public class AppointmentService {
    ...
    public boolean randevuAl(int doktorId, int hastaId, LocalDateTime tarih) {
        if (isSlotOccupied(doktorId, tarih)) {
            System.out.println("❌ HATA: Seçilen saat dolu!");
            return false;
        }
    ...
    public boolean randevuGuncelle(int randevuId, LocalDateTime yeniTarih) {
        ...
        if (isSlotOccupied(mevcut.getDoktorId(), yeniTarih)) return false;
    ...
    public List<Appointment> aramaVeFiltrele(int userId, LocalDate baslangic, LocalDate bitis, String aramaMetni, boolean isDoctor) {
        ...
        if (isDoctor) {
            sql.append("SELECT a.*, u.ad as h_ad, u.soyad as h_soyad, u.tc_kimlik_no as h_tc FROM appointments a ")
                    .append("JOIN users u ON a.hasta_id = u.id WHERE a.doktor_id = ? ");
        } else {
            sql.append("SELECT a.*, u.ad as dr_ad, u.soyad as dr_soyad, u.brans as dr_brans FROM appointments a ")
                    .append("JOIN users u ON a.doktor_id = u.id WHERE a.hasta_id = ? ");
        }
        ...
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            if (aramaMetni != null && !aramaMetni.trim().isEmpty()) {
                ...
                stmt.setString(index++, desen); stmt.setString(index++, desen);
                stmt.setString(index++, desen); stmt.setString(index++, desen);
            }
            ...
            while (rs.next()) {
                ...
                if (isDoctor) app.setHastaAdi(rs.getString("h_ad") + " " + rs.getString("h_soyad"));
                else {
    ...
    // DOKTOR İÇİN: İptal/Gelmedi Randevuları
    public List<Appointment> getDoktorIptalRandevular(int doktorId) {
        List<Appointment> liste = new ArrayList<>();
        String sql = "SELECT a.*, u.ad as h_ad, u.soyad as h_soyad, u.tc_kimlik_no as h_tc FROM appointments a " +
                "JOIN users u ON a.hasta_id = u.id " +
                "WHERE a.doktor_id = ? AND a.durum IN ('İPTAL EDİLDİ', 'GELMEDİ') " +
                "ORDER BY a.tarih DESC";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, doktorId);
    ...
    public List<String> getDoluSaatler(int doktorId, String tarihGun) {
        ...
        String sql = "SELECT DATE_FORMAT(tarih, '%H:%i') as saat FROM appointments WHERE doktor_id = ? AND DATE(tarih) = ? AND durum != 'İPTAL EDİLDİ'";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, doktorId);
